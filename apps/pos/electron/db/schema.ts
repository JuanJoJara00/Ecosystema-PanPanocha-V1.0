...
export const products = sqliteTable('products', {
    ...
    cost_price: real('cost_price').default(0), // Added: Profit Calc
    sku: text('sku'), // Added: Inventory Mgmt
    barcode: text('barcode'), // Added: Phase 5 Scanner
    tax_rate: real('tax_rate').default(0), // Added: Fiscal
    category: text('category'),
    active: integer('active', { mode: 'boolean' }).default(true),
    stock: integer('stock').default(0),
    min_stock: integer('min_stock'), // Added: Alerts
    max_stock: integer('max_stock'), // Added: Restock
    supplier_id: text('supplier_id'), // Added: Procurement
...
export const sales = sqliteTable('sales', {
    ...
    payment_data: text('payment_data'), // Added: JSON for card metadata
    tip_amount: real('tip_amount').default(0),
    discount_amount: real('discount_amount').default(0),
    discount_reason: text('discount_reason'), // Added: Audit
    diners: integer('diners').default(1),
    created_at: text('created_at').notNull(),
    created_by: text('created_by'),
    sale_channel: text('sale_channel'),
    source_device_id: text('source_device_id'), // Added: Audit
...
export const orders = sqliteTable('orders', {
    ...
    diners: integer('diners').default(1),
    cancellation_reason: text('cancellation_reason'), // Added: Loss Prevention
...
export const orderItems = sqliteTable('order_items', {
    ...
    notes: text('notes') // Added: Kitchen instructions
});
...
export const saleItems = sqliteTable('sale_items', {
    ...
    quantity: real('quantity').notNull(),
    unit_price: real('unit_price').notNull(),
    unit_cost: real('unit_cost').default(0), // Added: Profit Snapshot
    tax_amount: real('tax_amount').default(0), // Added: Fiscal
    total_price: real('total_price').notNull()
});
...
export const clients = sqliteTable('clients', {
    ...
    last_visit: text('last_visit'), // Added: Loyalty
    preferences: text('preferences'), // Added: JSON tags
    created_at: text('created_at'),
...
export const shifts = sqliteTable('shifts', {
    ...
    notes: text('notes'), // Added: Closing explanations
...
export const expenses = sqliteTable('expenses', {
    ...
    authorize_user_id: text('authorize_user_id'), // Added: Audit
    created_at: text('created_at'),
    synced: integer('synced', { mode: 'boolean' }).default(false)
});
...
export const rappiDeliveries = sqliteTable('rappi_deliveries', {
    ...
    status: text('status').default('pending'),
    delivery_code: text('delivery_code'), // Requested in Phase 3 Plan
    notes: text('notes'),
    created_at: text('created_at'),
    synced: integer('synced', { mode: 'boolean' }).default(false)
});
...
export const stockReservations = sqliteTable('stock_reservations', {
    ...
    product_id: text('product_id').notNull().references(() => products.id),
    quantity: real('quantity').notNull(),
    source_type: text('source_type').notNull(), // 'delivery', 'rappi', 'table'
