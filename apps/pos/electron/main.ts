...
import { products, sales, saleItems, orders, orderItems, shifts, expenses, tipDistributions } from './db/schema';
...
// DRIZZLE: db-save-sale
// Initialize Controllers
const db = getDb(); // DI Injection
const salesController = new SalesController(db);
const shiftController = new ShiftController(db);
...
const deliveryController = new DeliveryController(db);
const reservationController = new ReservationController(db);
const branchController = new BranchController(db);
